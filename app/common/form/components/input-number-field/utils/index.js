...
export const largeNumberParser = value => {
  const adjustedValue = value.replace(NOT_A_LARGE_NUMBER_REGEX, EMPTY_STRING).toLowerCase()
  const lastCharacter = last(adjustedValue)

  if (LARGE_NUMBER_LETTER_REGEX.test(lastCharacter)) return String(numeral(adjustedValue).value())

  return adjustedValue
}

// Separates every 3 digits by coma
export const largeNumberFormatter = pipe(String, replace(THREE_DIGITS_REGEX, COMMA))
